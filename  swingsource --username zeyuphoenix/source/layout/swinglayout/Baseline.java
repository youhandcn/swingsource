...
/**
 * Convenience class that can be used to determine the baseline of a particular
 * component. The static method <code>getBaseline</code> uses the following
 * algorithm to determine the baseline:
 * <ol>
 ...
 * <ol>
 * <li>If the Component defines the method getBaselineResizeBehaviorInt, the
 * return value from that method is used.
 * <li>If running on 1.6, the Component method getBaselineResizeBehavior is
 * invoked and the return value converted to one of the constants defined by
 * this class.
 * <li>If the component is one of the known Swing components,the baseline
 * resize behavior is calculated and returned.
 * <li>Otherwise, BRB_OTHER is returned.
 * </ol>
 * <p>
 * This class is primarily useful for JREs prior to 1.6. In 1.6 API for this was
 * added directly to Component. When run on 1.6 or newer, this class calls into
 * the appropriate Component methods.
 * 
...
public class Baseline {
	...
	// Map<Class,Method>
	private static final Map<Class<?>, Method> BASELINE_MAP = Collections
			.synchronizedMap(new HashMap<Class<?>, Method>(1));

	// Map<Class,Method> Method is getBaselineResizeBehaviorAsInt
	private static final Map<Class<?>, Method> BRB_I_MAP = Collections
			.synchronizedMap(new HashMap<Class<?>, Method>(1));
	 ...
	 * 
	 * @param c
	 *            the JComponent to get the baseline resize behavior for
	...
	/**
	 * Returns the baseline for the specified component, or -1 if the baseline
	 * can not be determined. The baseline is measured from the top of the
	 * component. This method returns the baseline based on the preferred size.
	 * 
	 * @param component
	...
	/**
	 * Returns the baseline for the specified component, or a value less than 0
	 * if the baseline can not be determined. The baseline is measured from the
	...
	private static int getTabbedPaneBaseline(JTabbedPane tp, int height) {
		if (tp.getTabCount() > 0) {
			...
			Insets tabAreaInsets = rotateInsets(UIManager
					.getInsets("TabbedPane.tabAreaInsets"), tp
					.getTabPlacement());
	...
	private static int getLabelBaseline(JLabel label, int height) {
		...
		SwingUtilities.layoutCompoundLabel(label, fm, "a", icon, label
	...
	/**
	 * Creates an instance of Baseline. You typically don't create a Baseline.
	 ...
	 * <code>getDefaults</code>. If you want to override the baseline suport
