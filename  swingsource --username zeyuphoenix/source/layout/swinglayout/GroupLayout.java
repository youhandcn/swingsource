 ...
 * achieve common, and not so common, layouts. Grouping is done by instances of
 * the Group class. GroupLayout supports two types of groups: <table>
 * <tr>
 * <td valign=top>Sequential:
 * <td>A sequential group positions its child elements sequentially, one after
 * another.
 * <tr>
 * <td valign=top>Parallel:
 * <td>A parallel group positions its child elements in the same space on top
 * of each other. Parallel groups can also align the child elements along their
 * baseline. </table> Each Group can contain any number of child groups,
 * Components or gaps. GroupLayout treats each axis independently. That is,
 * there is a group representing the horizontal axis, and a separate group
 * representing the vertical axis. The horizontal group is responsible for
 * setting the x and width of its contents, where as the vertical group is
 * responsible for setting the y and height of its contents.
 * <p>
 * The following code builds a simple layout consisting of two labels in one
 * column, followed by two textfields in the next column:
 * 
 * <pre>
 *   JComponent panel = ...;
 ...
 * <ul>
 * <li>The horizontal axis consists of a sequential group containing two
 * parallel groups. The first parallel group consists of the labels, with the
 * second parallel group consisting of the text fields.
 * <li>The vertical axis similarly consists of a sequential group containing
 * two parallel groups. The parallel groups align their contents along the
 * baseline. The first parallel group consists of the first label and text
 * field, and the second group consists of the second label and text field.
 * </ul>
 * There are a couple of things to notice in this code:
 * <ul>
 * <li>You need not explicitly add the components to the container, this is
 * indirectly done by using one of the <code>add</code> methods.
 * <li>The various <code>add</code> methods of <code>Groups</code> return
 * themselves. This allows for easy chaining of invocations. For example,
 * <code>group.add(label1).add(label2);</code> is equivalent to
 * <code>group.add(label1);group.add(label2);</code>.
 * <li>There are no public constructors for the Groups, instead use the create
 * methods of <code>GroupLayout</code>.
 * </ul>
 ...
 * <code>setAutocreateGaps()</code> method. Similarly you can use the
 * <code>setAutocreateContainerGaps()</code> method to insert gaps between the
...
public class GroupLayout implements LayoutManager2 {
	...
	/**
	 * Possible alignment type. Indicates the elements should be aligned to the
	 * origin. For the horizontal axis with a left to right orientation this
	...
	/**
	 * Possible alignment type. Indicates the elements should be aligned to the
	 * end. For the horizontal axis with a left to right orientation this means
	...
	/**
	 * Possible alignment type. Indicates the elements should centered in the
	 * spaced provided.
	 * 
	 * @see #createParallelGroup(int)
	 */
	...
	/**
	 * Possible value for the add methods that takes a Component. Indicates the
	 * size from the component should be used.
	 */
	public static final int DEFAULT_SIZE = -1;
	/**
	 * Possible value for the add methods that takes a Component. Indicates the
	 * preferred size should be used.
	 */
	...
	/**
	 * Group responsible for layout along the horizontal axis. This is NOT the
	 * user specified group, use getHorizontalGroup to dig that out.
	 */
	private Group horizontalGroup;
	/**
	 * Group responsible for layout along the vertical axis. This is NOT the
	 * user specified group, use getVerticalGroup to dig that out.
	 */
	...
	private static void checkResizeType(int type, boolean isComponentSpring) {
		if (type < 0
				&& ((isComponentSpring && type != DEFAULT_SIZE && type != PREFERRED_SIZE) || (!isComponentSpring && type != PREFERRED_SIZE))) {
	 ...
	 * non-visible components should not be treated as part of the layout. A
	 * value of <code>false</code> indicates that components should be
	 * positioned and sized regardless of visibility.
	 * <p>
	 * A value of <code>false</code> is useful when the visibility of
	 * components is dynamically adjusted and you don't want surrounding
	 * components and the sizing to change.
	 * <p>
	...
	/**
	 * Sets whether the component's visiblity is considered for sizing and
	 * positioning. A value of <code>Boolean.TRUE</code> indicates that if
	 * <code>component</code> is not visible it should not be treated as part
	 * of the layout. A value of <code>false</code> indicates that
	 * <code>component</code> is positioned and sized regardless of it's
	 * visibility. A value of <code>null</code> indicates the value specified
	 * by the single argument method <code>
	 * setHonorsVisibility</code> should
	 * be used.
	 * <p>
	 * If <code>component</code> is not a child of the <code>Container</code>
	 * this <code>GroupLayout</code> is managing, it will be added to the
	 * <code>Container</code>.
	 * 
	 * @param component
	 *            the component
	 * @param honorsVisibility
	 *            whether <code>component</code>'s visiblity should be
	...
	/**
	 * Returns a textual description of this GroupLayout. The return value is
	...
	private void dump(StringBuffer buffer, Spring spring, String indent,
		...
		buffer.append(indent + spring.getClass().getName() + " "
				+ Integer.toHexString(spring.hashCode()) + " " + origin
				+ ", size=" + spring.getSize() + ", alignment="
				+ spring.getAlignment() + " prefs=["
				+ spring.getMinimumSize(axis) + " "
				+ spring.getPreferredSize(axis) + " "
				+ spring.getMaximumSize(axis) + padding + "]\n");
	...
	/**
	 * Sets whether or not a gap between components should automatically be
	 * created. For example, if this is true and you add two components to a
	 * <code>SequentialGroup</code> a gap between the two will automatically
	 * be created. The default is false.
	 * 
	 * @param autocreatePadding
	 *            whether or not to automatically created a gap between
	 *            components and the container
	 */
	 ...
	 * components should automatically be created. The default is false.
	 * 
	 * @param autocreatePadding
	 *            whether or not to automatically create gaps between the
	...
	/**
	 * Returns whether or not gaps between the container and the first/last
	 * components should automatically be created. The default is false.
	 * 
	 * @return whether or not the gaps between the container and the first/last
	...
	/**
	 * Sets the <code>Group</code> that is responsible for layout along the
	 * horizontal axis.
	 * 
	 * @param group
	 *            <code>Group</code> responsible for layout along the
	 *            horizontal axis
	 * @throws IllegalArgumentException
	 *             if group is null
	 */
	...
	/**
	 * Returns the <code>Group</code> that is responsible for layout along the
	 * horizontal axis.
	 * 
	 * @return <code>ParallelGroup</code> responsible for layout along the
	 *         horizontal axis.
	 */
	 ...
	 * 
	 * @param group
	 *            <code>Group</code> responsible for layout along the vertical
	 *            axis.
	 * @throws IllegalArgumentException
	 *             if group is null.
	 */
	...
	/**
	 * Returns the <code>ParallelGroup</code> that is responsible for layout
	...
	/**
	 * Wraps the user specified group in a sequential group. If container gaps
	 * should be generate the necessary springs are added.
	 */
	 ...
	 * specifies how children elements should be positioned when the the
	 * parallel group is given more space than necessary. For example, if a
	 * ParallelGroup with an alignment of TRAILING is given 100 pixels and a
	 * child only needs 50 pixels, the child will be positioned at the position
	 * 50.
	 * 
	 * @param alignment
	 *            alignment for the elements of the Group, one of
	 *            <code>LEADING</code>, <code>TRAILING</code>,
	 *            <code>CENTER</code> or <code>BASELINE</code>.
	 * @throws IllegalArgumentException
	 *             if alignment is not one of <code>LEADING</code>,
	 *             <code>TRAILING</code>, <code>CENTER</code> or
	 *             <code>BASELINE</code>
	 ...
	 *            <code>CENTER</code> or <code>BASELINE</code>.
	 * @param resizable
	 *            whether or not the group is resizable. If the group is not
	 *            resizable the min/max size will be the same as the preferred.
	 * @throws IllegalArgumentException
	 *             if alignment is not one of <code>LEADING</code>,
	 *             <code>TRAILING</code>, <code>CENTER</code> or
	 *             <code>BASELINE</code>
	 ...
	 * @param anchorBaselineToTop
	 *            whether the baseline is anchored to the top or bottom of the
	 *            group
	...
	/**
	 * Forces the set of components to have the same size. This can be used
	 * multiple times to force any number of components to share the same size.
	 * <p>
	 ...
	 * @throws IllegalArgumentException
	 *             if <code>components</code> is null, or contains null.
	 */
	 ...
	 *            Components to force to have same size.
	 * @param axis
	 *            Axis to bind size, one of HORIZONTAL, VERTICAL or HORIZONTAL |
	 *            VERTICAL
	 * @throws IllegalArgumentException
	 *             if <code>components</code> is null, or contains null.
	 * @throws IllegalArgumentException
	 *             if <code>axis</code> does not contain
	...
	private void linkSize0(Component[] components, int axis) {
		LinkInfo master = getComponentInfo(components[components.length - 1])
				.getLinkInfo(axis);
	 ...
	 *            the Component that should be removed and replaced with
	 *            newComponent
	 * @param newComponent
	 *            the Component to put in existingComponents place
	 * @throws IllegalArgumentException
	 *             is either of the Components are null or if existingComponent
	...
	/**
	 * Sets the LayoutStyle this GroupLayout is to use. A value of null can be
	 * used to indicate the shared instance of LayoutStyle should be used.
	 * 
	 * @param layoutStyle
	...
	//
	/**
	 * Notification that a <code>Component</code> has been added to the parent
	 * container. Developers should not invoke this method directly, instead you
	 * should use one of the <code>Group</code> methods to add a
	 * <code>Component</code>.
	 * 
	 * @param name
	...
	/**
	 * Notification that a <code>Component</code> has been removed from the
	 * parent container. You should not invoke this method directly, instead
	 * invoke <code>remove</code> on the parent <code>Container</code>.
	 * 
	 * @param component
	 ...
	 * @throws IllegalArgumentException
	 *             if <code>parent</code> is not the same
	...
	public void layoutContainer(Container parent) {
		...
		Iterator<ComponentInfo> componentInfo = componentInfos.values()
				.iterator();
	...
	/**
	 * Notification that a <code>Component</code> has been added to the parent
	 * container. You should not invoke this method directly, instead you should
	 * use one of the <code>Group</code> methods to add a
	 * <code>Component</code>.
	 * 
	 * @param component
	 *            The component added
	...
	private void prepare(int sizeType) {
		...
		if (!isValid) {
			...
			for (Iterator<ComponentInfo> cis = componentInfos.values()
					.iterator(); cis.hasNext();) {
	...
	private void checkComponents() {
		Iterator<ComponentInfo> infos = componentInfos.values().iterator();
		while (infos.hasNext()) {
	...
	/**
	 * Adjusts the autopadding springs for the horizontal and vertical groups.
	 * If <code>insert</code> is true this will insert auto padding springs,
	 * otherwise this will only adjust the springs that comprise auto preferred
	 * padding springs.
	 */
	...
	/**
	 * Spring consists of a range: min, pref and max a value some where in the
	 * middle of that and a location. Subclasses must override methods to get
	 * the min/max/pref and will likely want to override the
	 * <code>setSize</code> method. Spring automatically caches the
	 * min/max/pref. If the min/pref/max has internally changes, or needs to be
	 * updated you must invoked clear.
	 */
	abstract class Spring {
		...
		/**
		 * Sets the value and location of the spring. Subclasses will want to
		 * invoke super, then do any additional sizing.
		 * 
		 * @param axis
		 ...
		 *            of this Spring
		 * @param size
		 *            of the Spring. If size is UNSET, this invokes clear.
		 */
		 ...
		 * NOT check the current size, rather it's meant to quickly test if this
		 * Spring will always have a zero size.
		 */
	 ...
	 * supported by <code>GroupLayout</code>: laying out components one after
	 * another (<code>SequentialGroup</code>) or layout on top of each other (<code>ParallelGroup</code>).
	 * Use one of <code>createSequentialGroup</code> or
	...
	public abstract class Group extends Spring {
		...
		/**
		 * This is invoked from <code>setSize</code> if passed a value other
		 * than UNSET.
		 */
		...
		/**
		 * Used to compute how the two values representing two springs will be
		 * combined. For example, a group that layed things out one after the
		 * next would return <code>a + b</code>.
		 */
		abstract int operator(int a, int b);

		/**
		 * Calculates the specified size. This is called from one of the
		 * <code>getMinimumSize0</code>, <code>getPreferredSize0</code> or
		 * <code>getMaximumSize0</code> methods. This will invoke to
		 * <code>operator</code> to combine the values.
		 */
		...
		// Padding
		/**
		 * Adjusts the autopadding springs in this group and its children. If
		 * <code>insert</code> is true this will insert auto padding springs,
		 * otherwise this will only adjust the springs that comprise auto
		 * preferred padding springs.
		 * 
		 * @param axis
		 *            the axis of the springs; HORIZONTAL or VERTICAL
		 * @param leadingPadding
		 *            List of AutopaddingSprings that occur before this Group
		 * @param trailingPadding
		 *            any trailing autopadding springs are added to this on exit
		 * @param leading
		 *            List of ComponentSprings that occur before this Group
		 * @param trailing
		 *            any trailing ComponentSpring are added to this List
		 * @param insert
		 *            Whether or not to insert AutopaddingSprings or just adjust
		 *            any existing AutopaddingSprings.
		 */
		abstract void insertAutopadding(int axis, List<Spring> leadingPadding,
				List<Spring> trailingPadding, List<Spring> leading,
				List<Spring> trailing, boolean insert);
	...
	/**
	 * A <code>Group</code> that lays out its elements sequentially, one after
	 * another. This class has no public constructor, use the
	...
	public class SequentialGroup extends Group {
		 ...
		 * 
		 * @param group
		 *            the <code>Group</code> to add
		 * @param useAsBaseline
		 *            whether the specified <code>Group</code> should be used
		 *            to calculate the baseline for this <code>Group</code>
		...
		/**
		 * Adds the specified Component. If the Component's min/max are
		 * different from its pref than the component will be resizable.
		 * 
		 * @param component
		 ...
		 * 
		 * @param useAsBaseline
		 *            whether the specified <code>Component</code> should be
		 *            used to calculate the baseline for this <code>Group</code>
		 * @param component
		...
		/**
		 * Adds the specified <code>Component</code>. Min, pref and max can
		 * be absolute values, or they can be one of <code>DEFAULT_SIZE</code>
		 * or <code>PREFERRED_SIZE</code>. For example, the following:
		 * 
		 * <pre>
		 * add(component, PREFERRED_SIZE, PREFERRED_SIZE, 1000);
		 * </pre>
		 * 
		 * Forces a max of 1000, with the min and preferred equalling that of
		 * the preferred size of <code>component</code>.
		 * 
		 * @param component
		 *            the Component to add
		 * @param min
		 ...
		 *            the maximum size
		 * @throws IllegalArgumentException
		 *             if min, pref or max are not positive and not one of
		 *             PREFERRED_SIZE or DEFAULT_SIZE
		 * @return this <code>SequentialGroup</code>
		 */
		public SequentialGroup add(Component component, int min, int pref,
				int max) {
			return (SequentialGroup) addSpring(new ComponentSpring(component,
					min, pref, max));
		}

		/**
		 * Adds a <code>Component</code> to this <code>Group</code> with the
		 * specified size.
		 * 
		 * @param useAsBaseline
		 *            whether the specified <code>Component</code> should be
		 ...
		 *            the <code>Component</code> to add
		 * @param min
		 *            the minimum size or one of <code>DEFAULT_SIZE</code> or
		 *            <code>PREFERRED_SIZE</code>
		 * @param pref
		 *            the preferred size or one of <code>DEFAULT_SIZE</code>
		 *            or <code>PREFERRED_SIZE</code>
		 * @param max
		 *            the maximum size or one of <code>DEFAULT_SIZE</code> or
		 *            <code>PREFERRED_SIZE</code>
		 * @return this <code>Group</code>
		 */
		public SequentialGroup add(boolean useAsBaseline, Component component,
		 ...
		 *            the size of the gap
		 * @throws IllegalArgumentException
		 *             if min < 0 or pref < 0 or max < 0 or the following is not
		 *             meant min <= pref <= max
		 * @return this <code>SequentialGroup</code>
		 */
		 ...
		 * 
		 * @param min
		 *            the minimum size of the gap, or PREFERRED_SIZE
		 * @param pref
		 ...
		 * @return this <code>SequentialGroup</code>
		 * @throws IllegalArgumentException
		 *             if <code>type</code> is not a valid LayoutStyle
		 *             constant
		 * @see LayoutStyle
		 */
		public SequentialGroup addPreferredGap(JComponent comp1,
				JComponent comp2, int type) {
		 ...
		 *            LayoutStyle
		 * @param canGrow
		 *            true if the gap can grow if more space is available
		 * @return this <code>SequentialGroup</code>
		 * @throws IllegalArgumentException
		 *             if <code>type</code> is not a valid LayoutStyle
		 *             constant
		 ...
		 */
		public SequentialGroup addPreferredGap(JComponent comp1,
				JComponent comp2, int type, boolean canGrow) {
			...
			return (SequentialGroup) addSpring(new PaddingSpring(comp1, comp2,
					type, canGrow));
		}
		 ...
		 * @throws IllegalArgumentException
		 *             if type is not one of <code>LayoutStyle.RELATED</code>
		 *             or <code>LayoutStyle.UNRELATED</code>
		 * @see LayoutStyle
		 */
		 ...
		 * That is, during layout the neighboring components are found, and the
		 * min of this element is set based on the preferred gap between the
		 * components. If no neighboring components are found the min is set to
		 * 0. This method allows you to specify the preferred and maximum size
		 * by way of the <code>pref</code> and <code>max</code> arguments.
		 * These can either be a value &gt;= 0, in which case the preferred or
		 * max is the max of the argument and the preferred gap, of
		 * DEFAULT_VALUE in which case the value is the same as the preferred
		 * gap.
		 * 
		 * @param type
		 ...
		 * @param pref
		 *            the preferred size; one of DEFAULT_SIZE or a value > 0
		 * @param max
		 *            the maximum size; one of DEFAULT_SIZE, PREFERRED_SIZE or a
		 *            value > 0
		 * @return this SequentialGroup
		 * @throws IllegalArgumentException
		 *             if type is not one of <code>LayoutStyle.RELATED</code>
		 *             or <code>LayoutStyle.UNRELATED</code> or pref/max is !=
		...
		public SequentialGroup addPreferredGap(int type, int pref, int max) {
			...
			return (SequentialGroup) addSpring(new AutopaddingSpring(type,
					pref, max));
		}

		/**
		 * Adds an element representing the preferred gap between one edge of
		 * the container and the next/previous Component. This will have no
		 * effect if the next/previous element is not a Component and does not
		 * touch one edge of the parent container.
		 * 
		 * @return this <code>SequentialGroup</code>.
		 */
		...
		public SequentialGroup addContainerGap(int pref, int max) {
			...
			hasPreferredPaddingSprings = true;
			return (SequentialGroup) addSpring(new ContainerAutopaddingSpring(
					pref, max));
		}
		...
		void setValidSize(int axis, int origin, int size) {
			...
			if (size == pref) {
				...
				spring.setSize(axis, origin, Math.min(Math.max(size, spring
						.getMinimumSize(axis)), spring.getMaximumSize(axis)));
			} else if (springs.size() > 1) {
		...
		private void setValidSizeNotPreferred(int axis, int origin, int size) {
			...
			if (resizableCount > 0) {
				...
				for (int counter = 0; counter < resizableCount; counter++) {
					...
					if (springDelta.delta != sDelta
							&& counter + 1 < resizableCount) {
		...
		private int indexOfNextNonZeroSpring(int index,
				boolean treatAutopaddingAsZeroSized) {
			while (index < springs.size()) {
				Spring spring = (Spring) springs.get(index);
				if (!((Spring) spring)
						.willHaveZeroSize(treatAutopaddingAsZeroSized)) {
					return index;
				}
				index++;
			}
		...
		void insertAutopadding(int axis, List<Spring> leadingPadding,
				List<Spring> trailingPadding, List<Spring> leading,
				List<Spring> trailing, boolean insert) {
			List<Spring> newLeadingPadding = new ArrayList<Spring>(
					leadingPadding);
			List<Spring> newTrailingPadding = new ArrayList<Spring>(1);
			List<Spring> newLeading = new ArrayList<Spring>(leading);
			List<Spring> newTrailing = null;
			int counter = 0;
			...
			while (counter < springs.size()) {
				...
				if (spring instanceof AutopaddingSpring) {
					...
					if (spring instanceof ComponentSpring) {
						...
						if (newTrailing == null) {
							newTrailing = new ArrayList<Spring>(1);
						} else {
							newTrailing.clear();
						}
	 ...
	 */
	private static final class SpringDelta implements Comparable<Object> {
		// Original index.
	...
	/**
	 * A <code>Group</code> that lays out its elements on top of each other.
	 * If a child element is smaller than the provided space it is aligned based
	 * on the alignment of the child (if specified) or on the alignment of the
	 * ParallelGroup.
	 * 
	...
	public class ParallelGroup extends Group {
		...
		public ParallelGroup add(Component component, int min, int pref, int max) {
			return (ParallelGroup) addSpring(new ComponentSpring(component,
					min, pref, max));
		}
		 ...
		 * @throws IllegalArgumentException
		 *             if alignment is not one of <code>LEADING</code>,
		 *             <code>TRAILING</code> or <code>CENTER</code>
		 */
		...
		public ParallelGroup add(int alignment, Component component, int min,
				int pref, int max) {
			checkChildAlignment(alignment);
			ComponentSpring spring = new ComponentSpring(component, min, pref,
					max);
		...
		void setChildSize(Spring spring, int axis, int origin, int size) {
			...
			int springSize = Math.min(Math.max(spring.getMinimumSize(axis),
					size), spring.getMaximumSize(axis));
			...
			case TRAILING:
				...
				break;
			case CENTER:
				spring.setSize(axis, origin + (size - springSize) / 2,
						springSize);
				break;
				...
				if (spring instanceof ComponentSpring) {
					if (((ComponentSpring) spring).isVisible()) {
						for (int i = 0; i < leadingPadding.size(); i++) {
							((AutopaddingSpring) leadingPadding.get(i))
									.addTarget((ComponentSpring) spring, axis);
						}
						trailing.add(spring);
					}
				} else if (spring instanceof Group) {
	...
	/**
	 * An extension of <code>ParallelGroup</code> that aligns its constituent
	 * <code>Spring</code>s along the baseline.
	 */
	private class BaselineGroup extends ParallelGroup {
		...
		int calculateSize(int axis, int type) {
			...
			return Math.max(prefAscent + prefDescent, super.calculateSize(axis,
					type));
		}
		...
		private int calculateMaxSize() {
			...
			for (int counter = springs.size() - 1; counter >= 0; counter--) {
				...
				if ((spring.getAlignment() == NO_ALIGNMENT || spring
						.getAlignment() == BASELINE)
					...
					if (springPref != springMax) {
						...
						case Baseline.BRB_CONSTANT_ASCENT:
							if (baselineAnchoredToTop) {
								maxDescent = Math.max(maxDescent, springMax
										- baseline);
							}
							break;
						case Baseline.BRB_CONSTANT_DESCENT:
							if (!baselineAnchoredToTop) {
								maxAscent = Math.max(maxAscent, springMax
										- springPref + baseline);
							}
							break;
		...
		private int calculateMinSize() {
			...
			for (int counter = springs.size() - 1; counter >= 0; counter--) {
				...
				if ((spring.getAlignment() == NO_ALIGNMENT || spring
					...
					case Baseline.BRB_CONSTANT_DESCENT:
						if (!baselineAnchoredToTop) {
							minAscent = Math.max(baseline
									- (springPref - springMin), minAscent);
						} else {
					...
					default:
						...
						minAscent = Math.max(baseline, minAscent);
						minDescent = Math
								.max(springPref - baseline, minDescent);
						break;
					}
				} else {
					// Not aligned along the baseline, or no baseline.
		...
		/**
		 * Lays out springs that have a baseline along the baseline. All others
		 * are centered.
		 */
				...
				if (alignment == NO_ALIGNMENT || alignment == BASELINE) {
					...
					if (baseline >= 0) {
						...
						case Baseline.BRB_CONSTANT_ASCENT:
							...
							height = Math.min(descent, springMax - baseline)
									+ baseline;
							break;
						case Baseline.BRB_CONSTANT_DESCENT:
							height = Math.min(ascent, springMax - springPref
									+ baseline)
									+ (springPref - baseline);
							y = origin + ascent + (springPref - baseline)
									- height;
							break;
	...
	/**
	 * A Spring representing one axis of a Component. There are three ways to
	 * configure this:
	 * <ul>
	 * <li>Use the pref/min/max from the component
	 * <li>Use the pref from the component and fix the min to 0 or max to a big
	 * number.
	 * <li>Force the min/max/pref to be a certain value. If the Component's
	 * size is to be linked to another components than the min/max/pref all come
	...
	private final class ComponentSpring extends Spring {
		...
		int getBaseline() {
			if (baseline == -1 && (component instanceof JComponent)) {
				...
				if (width > 0 && height > 0) {
					baseline = Baseline.getBaseline((JComponent) component,
	...
	private final class PaddingSpring extends Spring {
		...
		private int getPadding(int axis) {
			...
			return getLayoutStyle0().getPreferredGap(source, target, type,
					position, host);
		}
	 ...
	 * targets. The targets and sources are computed during layout. An instance
	 * of this can either be dynamically created when autocreatePadding is true,
	 * or explicitly created by the developer.
	 */
	private class AutopaddingSpring extends Spring {
		List<Spring> sources;
		ComponentSpring source;
		private List<AutopaddingMatch> matches;
		int size;
		int lastSize;
		private final int pref;
		private final int max;
		...
		public void setSources(List<Spring> sources) {
			this.sources = new ArrayList<Spring>(sources);
		}
		...
		public void calculatePadding(int axis) {
			...
			if (matches != null) {
				...
				for (int i = matches.size() - 1; i >= 0; i--) {
					...
					maxPadding = Math.max(maxPadding, calculatePadding(p,
							position, match.source, match.target));
				}
			}
			if (size == UNSET) {
				size = 0;
		...
		private int calculatePadding(LayoutStyle p, int position,
				ComponentSpring source, ComponentSpring target) {
			int delta = target.getOrigin()
					- (source.getOrigin() + source.getSize());
			if (delta >= 0) {
				int padding;
				if ((source.getComponent() instanceof JComponent)
						&& (target.getComponent() instanceof JComponent)) {
					padding = p.getPreferredGap((JComponent) source
							.getComponent(),
		...
		public void addTarget(ComponentSpring spring, int axis) {
			...
			if (source != null) {
				if (areParallelSiblings(source.getComponent(), spring
						.getComponent(), oAxis)) {
				...
				for (int counter = sources.size() - 1; counter >= 0; counter--) {
					ComponentSpring source = (ComponentSpring) sources
							.get(counter);
					if (areParallelSiblings(source.getComponent(), component,
							oAxis)) {
						addValidTarget(source, spring);
					}
				}
		...
		private void addValidTarget(ComponentSpring source,
			...
			if (matches == null) {
				matches = new ArrayList<AutopaddingMatch>(1);
			}
	...
	private class ContainerAutopaddingSpring extends AutopaddingSpring {
			...
			if (targets != null) {
				...
				for (int i = targets.size() - 1; i >= 0; i--) {
					...
					if (targetSpring.getComponent() instanceof JComponent) {
						padding = p.getContainerGap((JComponent) targetSpring
								.getComponent(), position, host);
				...
				if (sources != null) {
					for (int i = sources.size() - 1; i >= 0; i--) {
						ComponentSpring sourceSpring = (ComponentSpring) sources
								.get(i);
						maxPadding = Math.max(maxPadding, updateSize(p,
								sourceSpring, position));
					}
				} else if (source != null) {
		...
		private int updateSize(LayoutStyle p, ComponentSpring sourceSpring,
			...
			if (sourceSpring.getComponent() instanceof JComponent) {
				padding = p.getContainerGap((JComponent) sourceSpring
						.getComponent(), position, host);
			}
	...
	private static final class LinkInfo {
		...
		public void add(ComponentInfo child) {
			...
			if (childMaster == null) {
				...
				for (int i = 0; i < childMaster.linked.size(); i++) {
					ComponentInfo childInfo = (ComponentInfo) childMaster.linked
							.get(i);
		...
		private int calculateLinkedSize(int axis) {
			...
			for (int i = 0; i < linked.size(); i++) {
				...
				size = Math.max(size, spring
						.calculateNonlinkedPreferredSize(axis));
			}
			return size;
		}
	}
	...
	/**
	 * Tracks the horizontal/vertical Springs for a Component. This class is
	 * also used to handle Springs that have their sizes linked.
	 */
	private final class ComponentInfo {
		...
		/**
		 * Returns true if this component has its size linked to other
		 * components.
		 */
