...
/**
 * <p>
 * TableLayout是一种比GridBagLayout使用简单并且功能强大的布局管理器.
 * </p>
 * 
 * <p>
 * TableLayout是一种把控件按电子表格的行和列的样式排列的布局管理器.TableLayout允许
 * 每个行和列都有不同大小.每个行或者列都能被设置为一个绝对的像素大小,赋予使用的空间一个
 * 百分比,当其它的行和列改变尺寸之后，这个行或列可以扩大和缩小以填满余下的空白空间.
 * </p>
 * 
 * <p>
 ...
 * 通常对于电子数据表格来说,一个单元格就是一个行和列的交汇
 * </p>
 * 
 * <p>
 * A component occupies a rectangular group of one or more cells. The component
 * can be aligned within those cells using four vertical and six horizontal
 * justifications. The vertical justifications are left, center, right, and
 * full. The horizontal justifications are left, center, right, full, leading,
 * and trailing. With full justification the component is stretched either
 * vertically or horizontally to fit the cell or group of cells.
 * <p>
 * 
 * <b>Justification</b>
 * 
 * <p>
 * Leading and trailing justification are used to support languages that are
 * read from right to left. See the <code>java.awt.ComponentOrientation</code>
 * class for details and http://java.sun.com/products/jfc/tsc/articles/bidi for
 * an introduction to component orientation and bidirectional text support. The
 * leading justification will align the component along the leading edge of the
 ...
 * vertical axis since all modern languages are read from top to bottom and no
 * bottom-to-top orientation is defined in
 * <code>java.awt.ComponentOrientation.</code>
 * </p>
 * 
 * <p>
 * For components using the <code>ComponentOrientation.LEFT_TO_RIGHT</code>
 ...
 * 
 * <p>
 * 行和列的水平和垂直的间隔的设置有2种方法.如果有设置间隔的需求，可以使用setHGap和setVGap方法.
 * 如果需要改变间隔的大小,简单的话使用绝对大小的空白的行和列.同样的,需要实现一个容器的边缘环绕
 * 也不需要插入控件,使用空白的行和列沿着容器的边一周就可以了.
 * </p>
 * 
 * <p>
 * 使用TableLayout只需要简单的2个步骤.首先,通过使用任何一个TableLayout的构造器或者设置行和列
 * 的方法指定具体的行和列的大小为你的容器创建一个表格.第二,通过指定的行和列添加控件到单元格.
 * </p>
 * 
 * <p>
 * When adding a component to a container that uses TableLayout, you specify the
 * component's constraints that state which cells the component will occupy and
 * how the component will be aligned. The constraints can be specified into two
 * ways. The <code>TableLayoutConstraints</code> class can be used to
 * systematically specify the constraints. This is useful to dynamic code, bean
 * builders, and rapid application development software.
 * </p>
 * 
 * <p>
 * For manual coding, a quicker and easier way to specify constraints is with a
 * short string in the form "x1, y1, x2, y2, hAlign, vAlign" where (x1, y1)
 * identifies the top left cell (column x1, row y1) for the component and (x2,
 * y2) identifies the bottom right cell. x2 and y2 are optional. If they are not
 * specified, the component will occupy only one cell, (x1, y1). hAlign and
 * vAlign are also optional with default values of full justification.
 * Alignments may be spelt fully as in "LEFT" or abbreviated as in "L". The text
 * is not case sensitive, but it is recommended that uppercase is used for two
 * reasons. First, these text values are in essence constants. Second, some
 * fonts use the same glyphs for representing a lowercase L and the number one.
 * Ex., "l" vs. "1". Even fonts that do not will often use similar glyphs so
 * using uppercase avoids confusion.
 * </p>
 * 
 * <p>
 * 行和列可以在任何时间、任何时候甚至是容器不可见时被动态创建，改变尺寸和移除.当单元格在电子
 * 表格中改变时,控件将进行合适的改变.
 * </p>
 * 
 * <p>
 * 行和列可以通过设置它们的大小为0来实现隐藏或者消失效果.它们也可以通过重新设置非0大小来再次显示.
 * 这是不移除特定控件来改变布局的非常有用的方法.
 * </p>
 * 
 * <b>Preferred sizes</b>
 * 
 ...
 * 
 * <p>
 * There is no corresponding <code>MAXIMUM</code> constant for several
 * reasons. First, it is mathematically impossible to honor both the minimum and
 * maximum sizes of more than one component when conflicts arise. For example,
 * say components a and b are in the same row. If a's maximum height is less
 * than b's minimum height, then one of these constraints must be violated.
 * Since TableLayout is a complete, general Cartesian layout manager, it would
 * be possible to specify conflicting constraints if a <code>MAXIMUM
 * </code>
 * constant existed.
 * <p>
 * 
 ...
 * primarily of interest to layout managers like <code>SpringLayout</code>
 * that have to balance the sizes of components because the presence of one
 * component affects the size of another. Other than the effect of preferred and
 * minimum size rows/columns, which are essentially convenient ways of
 * specifying absolute sizes, the existence and constraints of one component
 * does not affect any other components when using TableLayout. This is
 * accomplished because rows and columns are explicit in TableLayout.
 * </p>
 * 
 * <p>
 * Third, the ability to constrain a component to its maximum size is subsumed
 * by the ability to constrain it to its preferred size, which is precisely what
 * happens when a component is aligned using anything but full justification. In
 * the case of full justification, the component's maximum size is by definition
 * unbounded.
 * </p>
 */
public class TableLayout implements LayoutManager2, Serializable,
			...
			if (crSpec[z][counter] == typeOfSize) {
				...
				nextComponent: while (iterator.hasNext()) {
					...
					if ((entry.cr1[z] <= counter) && (entry.cr2[z] >= counter)) {
						...
						Dimension p = (typeOfSize == PREFERRED) ? entry.component
								.getPreferredSize()
						...
						// Adjust remaining size by eliminating space for gaps
						// between the crs this component occupies
						int numCrOccupiedByThisComponent = entry.cr2[z]
								- entry.cr1[z] + 1;
	...
	protected int[] calculateSizeAndOffset(Entry entry, int preferredSize,
		...
		int cellSetSize = isColumn ? crOffset[entry.cr2[C] + 1]
				- crOffset[entry.cr1[C]] : crOffset[entry.cr2[R] + 1]
	...
	protected Dimension calculateLayoutSize(Container container,
		...
		for (int i = 0; i < numEntry; i++)
			prefMinSize[i] = (typeOfSize == PREFERRED) ? entryList[i].component
	...
	public static class Entry implements Cloneable {
		 ...
		 * 
		 * @return a string in the form "(col1, row1, col2, row2, vAlign,
		 *         hAlign) component"
		 */
